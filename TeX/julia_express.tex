...
If you prefer to read this document in a different format than PDF then the simplest approach to do it is to
clone the project repository \url{https://github.com/bkamins/The-Julia-Express} to your local folder and use
...
\begin{lstlisting}
  ...
  h(x...) = sum(x)/length(x) - mean(x) # a vararg function; x is a tuple; need to run first: using Statistics
