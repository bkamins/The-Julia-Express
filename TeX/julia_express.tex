...
All suggestions how this guide can be improved are welcomed. Please contact me
at \href{mailto:bkamins@sgh.waw.pl}{bkamins@sgh.waw.pl}.
...
assertion will fail on 64-bit Julia.

Notably \lstinline|Int| is a constant whose
...
it. This is especially important in function calls. The simplest, and
preferred, way to perform the construction of a value \lstinline|x| to type
\lstinline|T| by writing \lstinline|T(x)|, for example:
\begin{lstlisting}
...
\end{lstlisting}
Conversion can be done using \lstinline|convert(Type, x)| (typically
\lstinline|convert| will not perform a copy if \lstinline|x| already has type
\lstinline|Type|; for this reason it is typically preferable to perform
construction, as it was explained above, rather than conversion):
\begin{lstlisting}
...
\end{lstlisting}
Use \lstinline|floor(Int64, 1.3)|, \lstinline|ceil(Int64, 1.3)| or
\lstinline|round(Int64, 1.3)| to perform rounding.
...
achieved using \lstinline|promote| (typically you will not need to perform such
promotion yourself):
\begin{lstlisting}
...
An important feature is that \verb|Bool| is considered to be \verb|Integer|.
This is useful for performing arithmetic operations, but sometimes can be
surprising.
...
\verb|Ref| is a special 0-dimensional container that can store any value:
\begin{lstlisting}
...
Note that \lstinline|for|, \lstinline|while|, \lstinline|try|, and \lstinline|struct|
use a so called \emph{soft} local scope. Simplifying a bit this means that if they
are used in a top level (global) scope they overwrite existing global variables
if you are in an interactive mode (REPL, notebook):
...
However, the same code passed in an non-interactive session prints a warning
and does not overwrite a global variable:
...
\end{lstlisting}
Use BenchmarkTools.jl package (version 1.3.2) for a more powerful benchmarking
functionality.
...
An example reproducing \url{https://matplotlib.org/1.2.1/examples/pylab_examples/histogram_demo.html}
in which we also use StatsPlots.jl package (version 0.15.4):
\begin{lstlisting}
  using Distributions
  using Plots
  using Random
...
Here we will show how DataFrames.jl (version 1.4.4)
and CSV.jl (version 0.10.8) packages can be used.
...
this$\quad$
\includegraphics[height=55mm]{heart_in.png}
$\quad$to this$\quad$
\includegraphics[height=55mm]{heart_out.png}
